...
contract HostelRental {
    ...
    event Rent(address renter, uint rentalPeriod, uint amountPaid);
    
    constructor(uint _rentalPrice, uint _deposit) {
        owner = msg.sender;
        rentalPrice = _rentalPrice;
        deposit = _deposit;
    }
    
    function rent(uint rentalPeriod) public payable {
        require(msg.value == rentalPrice * rentalPeriod + deposit, "Insufficient payment");
        renters[msg.sender] = block.timestamp + rentalPeriod * 1 days;
        emit Rent(msg.sender, rentalPeriod, msg.value);
    }
    
    function checkRental(address renter) public view returns(uint) {
        return renters[renter];
    }
    
    function terminateRental() public {
        require(msg.sender == owner || renters[msg.sender] > 0, "Unauthorized access");
        if(msg.sender == owner) {
            payable(owner).transfer(address(this).balance);
        } else {
            require(block.timestamp >= renters[msg.sender], "Rental period has not ended");
